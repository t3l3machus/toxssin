...
toxssin is an open-source penetration testing tool that automates the process of exploiting Cross-Site Scripting (XSS) vulnerabilities. It consists of an https server that works as a "play maker" and interpreter for the traffic generated by the malicious JavaScript payload served by the tool (toxin.js).This project started as (and still is) a research attempt to see how far can a reflected or stored XSS be exploited using vanilla JavaScript and cheap tricks.

The project is quite fresh and has not been widely tested, as subjects (XSS vulnerable applications)   
